...
def alignment_loss(teacher_hidden_states: torch.Tensor, student_hidden_states: Union[List[torch.Tensor], torch.Tensor]):
    ...
    elif teacher_hidden_states.dim() == 2: # [num_align_in_a_seg, dim], align last layer
        if isinstance(student_hidden_states, list):
            student_last = student_hidden_states[-1]
        elif student_hidden_states.dim() == 3:
            student_last = student_hidden_states[-1]
        else:
            student_last = student_hidden_states
        total_loss = (1 - torch.nn.functional.cosine_similarity(student_last.to(teacher_hidden_states.device), teacher_hidden_states)).mean()
...
class Qwen2_5_VisionTransformerPretrainedModel(Qwen2_5_VLPreTrainedModel):
    ...
    def rot_pos_emb(self, grid_thw):
        ...
        # Handle case when there are no images (empty grid_thw)
        if len(pos_ids) == 0:
            # Return empty tensor with correct shape
            return torch.empty(0, self.embed_dim, device=grid_thw.device, dtype=torch.float32)
    ...
    def get_window_index(self, grid_thw):
        ...
        if len(window_index) == 0:
            # Return empty tensor and cu_window_seqlens
            return torch.empty(0, dtype=torch.long, device=grid_thw.device), cu_window_seqlens
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        if grid_thw.shape[0] == 0 or hidden_states.shape[0] == 0:
            # Return empty tensor with correct shape
            return torch.empty(0, self.embed_dim, device=hidden_states.device, dtype=hidden_states.dtype)
...
class Qwen2_5_VLTextModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if not return_dict:
            return tuple(
                v for v in [hidden_states, past_key_values, batch_last_hidden_state_states, all_self_attns] if v is not None
            )
        return BaseModelOutputWithPast(
            last_hidden_state=hidden_states,
            past_key_values=past_key_values,
            hidden_states=batch_last_hidden_state_states,
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        second_per_grid_ts (`torch.Tensor` of shape `(num_videos)`, *optional*):
            The time interval (in seconds) for each grid along the temporal dimension in the 3D position IDs.
        attention_mask_4d (`torch.Tensor`, *optional*):
            4D boolean causal mask (expects key `full_attention`) for restricting thought-image visibility.
        latent_mode (`bool`, *optional*):
            If True, run the latent-token first pass to produce cached latent embeddings and alignment loss.
        alignment_poss (`List[List[int]]`, *optional*):
            Latent token positions in each sample, used for alignment and cache patching.
        teacher_hidden_states_for_alignment (`torch.Tensor` or list, *optional*):
            Teacher targets for alignment loss; supports shapes `[K, H]` or `[num_layer, K, H]` per sample.
        ce_patch_pos (`List[List[int]]`, *optional*):
            Token positions to be patched with latent embeddings during CE forward.
        ce_patch_vec (`List[torch.Tensor]`, *optional*):
            Latent embeddings corresponding to `ce_patch_pos` for CE forward.
        """
        ...
        if latent_mode: # latent forward, SFT Stage 2 student CoT or SFT Stage 3
            ...
            for b in range(batch_size):
                ...
                teacher_align_dim = None
                if teacher_hidden_states_for_alignment is not None:
                    t_align = teacher_hidden_states_for_alignment[b]
                    teacher_align_dim = t_align.dim()
                    if teacher_align_dim == 2:
                        assert t_align.shape[0] == len(align_pos), f"teacher_hidden_states_for_alignment.shape[0] ({t_align.shape[0]}) != len(align_pos) ({len(align_pos)})"
                    elif teacher_align_dim == 3:
                        assert t_align.shape[1] == len(align_pos), f"teacher_hidden_states_for_alignment.shape[1] ({t_align.shape[1]}) != len(align_pos) ({len(align_pos)})"
                    else:
                        raise ValueError(f"Unexpected teacher_hidden_states_for_alignment dim {teacher_align_dim}; expected 2 or 3")

                seq_embeds   = inputs_embeds[b : b + 1]         # (1,L,H)
                pos_ids_s    = position_ids[:, b : b + 1]       # (3,1,L)
                if attention_mask_4d is not None:
                    attn_mask_s = {'full_attention': attention_mask_4d['full_attention'][b : b + 1]}  
                else: 
                    attn_mask_s = attention_mask[b: b + 1]
                        ...
                        if cut > s:
                            def _run_text_chunk(q0:int, q1:int, k1:int, past_kv):
                                ...
                                out = run_segment_forward(
                                    ...
                                    seg_att_m=seg_att_m,
                                    past_kv=past_kv,
                                    need_hidden=(teacher_align_dim == 3),
                                    no_grad_mode=False,
                                    **kwargs,
                                )
                                return out
                            ...
                            # ------------------------------------------------------------------
                            if teacher_align_dim == 3: # align all layers
        ...
        # ---------- END latent_mode implementation ----------
            ...
            need_alignment = "alignment" in kwargs.get('loss_type', {})
            align_teacher_dim = None
            if teacher_hidden_states_for_alignment is not None:
                align_teacher_dim = teacher_hidden_states_for_alignment[0].dim()
            ...
            # Only request per-layer hidden states when teacher provides per-layer targets.
            if need_alignment and align_teacher_dim == 3:
                output_hidden_states = True
            ...
            if need_alignment:
                ...
                elif align_teacher_dim == 2:
                    for b in range(batch_size):
                        student_last_hidden = outputs.last_hidden_state[b, alignment_poss[b], :]
                        total_align_loss += alignment_loss(
                                                teacher_hidden_states_for_alignment[b], # [num_align_in_a_seg, dim]
                                                student_last_hidden, # [num_align_in_a_seg, dim]
                                            )
                else:
                    total_align_loss = None
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        **kwargs: Unpack[TransformersKwargs],
        ...
        ce_patch_vec (`List[torch.Tensor]`, *optional*):
            Latent embeddings corresponding to `ce_patch_pos` for CE forward.
        ce_emphasize_factor (`float`, *optional*):
            Weighting factor for emphasized CE positions (if provided).
        ce_emphasize_poss (`List[List[int]]`, *optional*):
            Token positions to up-weight in CE loss when `ce_emphasize_factor` != 1.
        loss_type (`List[str]`, *optional*):
            Which losses to compute; supports `\"ce\"` and `\"alignment\"`.
        output_latent_embeds (`bool`, *optional*):
            If True and `latent_mode` is set, also return latent embeddings.
        compute_emphasize_acc (`bool`, *optional*):
            If True, report accuracy on `ce_emphasize_poss`.
        ...
        # ----------------------------
        # Loss mode compatibility
        # ----------------------------
        # TRL/Transformers call the model with `labels=...` for standard LM training.
        # Our latent extension introduced `loss_type` to switch between CE/alignment.
        # To remain backward compatible, if `labels` is provided and the caller did not
        # explicitly request a loss type, we default to CE.
        if loss_type is None:
            loss_type = []
        if not isinstance(loss_type, (list, tuple)):
            loss_type = []
        loss_type = list(loss_type)
        if labels is not None and "ce" not in loss_type:
            loss_type = ["ce"] + loss_type
        ...
        # Always compute logits (needed for both training and inference/generation)
